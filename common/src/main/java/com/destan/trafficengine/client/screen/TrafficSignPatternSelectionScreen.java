    ...
    private void loadTextures() {
        ...
        if (selectedShape != null && bookmarks.contains(selectedShape)) {
            ...
            for (TrafficSignShape shape : shapes) {
                ...
                int maxBuiltInId = shape == TrafficSignShape.TRIANGLE ? 47 : Integer.MAX_VALUE;
                while (a <= maxBuiltInId && Minecraft.getInstance().getResourceManager().getResource(path).isPresent()) {
                    short width = 32;
                    short height = 32;
                    ...
                    boolean hiddenYellowCircle = shape == TrafficSignShape.CIRCLE && (
                            (a >= 74 && a <= 112)
                            || (a >= 118 && a <= 122)
                            || (a >= 128 && a <= 132)
                            || (a >= 138 && a <= 141));
                    boolean hiddenSquareRange = shape == TrafficSignShape.SQUARE && a >= 96 && a <= 106;
                    if (!hiddenYellowCircle && !hiddenSquareRange) {
